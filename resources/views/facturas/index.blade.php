...
<x-filtros-avanzados :action="route('facturas.index')">
    <div class="vx-filtro" data-filtro="codigo"><label class="vx-filtro-label">Código</label><select name="codigo_factura" class="vx-select"><option value="">Todos</option>@foreach($codigos_factura as $c)<option value="{{ $c }}" {{ request('codigo_factura') == $c ? 'selected' : '' }}>{{ $c }}</option>@endforeach</select></div>
    ...
    <div class="vx-filtro" data-filtro="concepto"><label class="vx-filtro-label">Concepto</label><select name="concepto" class="vx-select"><option value="">Todos</option>@foreach($conceptos_factura as $c)<option value="{{ $c }}" {{ request('concepto') == $c ? 'selected' : '' }}>{{ $c }}</option>@endforeach</select></div>
    <div class="vx-filtro" data-filtro="total_min"><label class="vx-filtro-label">Total mín.</label><input type="number" step="0.01" name="total_min" class="vx-input" value="{{ request('total_min') }}"></div>
    <div class="vx-filtro" data-filtro="total_max"><label class="vx-filtro-label">Total máx.</label><input type="number" step="0.01" name="total_max" class="vx-input" value="{{ request('total_max') }}"></div>
...
<div class="vx-card"><div class="vx-card-body" style="padding:0;">
    ...
    <div class="vx-table-wrapper"><table class="vx-table">
        <thead><tr><x-columna-ordenable campo="codigo_factura" label="Código" /><x-columna-ordenable campo="cliente_id" label="Cliente" /><x-columna-ordenable campo="marca_id" label="Marca" /><x-columna-ordenable campo="concepto" label="Concepto" /><x-columna-ordenable campo="subtotal" label="Subtotal" /><x-columna-ordenable campo="iva_importe" label="IVA" /><x-columna-ordenable campo="total" label="Total" /><x-columna-ordenable campo="estado" label="Estado" /><x-columna-ordenable campo="fecha_factura" label="Fecha" /><th>Acciones</th></tr></thead>
