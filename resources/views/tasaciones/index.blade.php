...
<x-filtros-avanzados :action="route('tasaciones.index')">
    <div class="vx-filtro" data-filtro="codigo"><label class="vx-filtro-label">Código</label><select name="codigo_tasacion" class="vx-select"><option value="">Todos</option>@foreach($codigos_tasacion as $c)<option value="{{ $c }}" {{ request('codigo_tasacion') == $c ? 'selected' : '' }}>{{ $c }}</option>@endforeach</select></div>
    ...
    <div class="vx-filtro" data-filtro="km_min"><label class="vx-filtro-label">Km mín.</label><input type="number" name="km_min" class="vx-input" value="{{ request('km_min') }}"></div>
    <div class="vx-filtro" data-filtro="km_max"><label class="vx-filtro-label">Km máx.</label><input type="number" name="km_max" class="vx-input" value="{{ request('km_max') }}"></div>
    <div class="vx-filtro" data-filtro="matricula"><label class="vx-filtro-label">Matrícula</label><select name="matricula" class="vx-select"><option value="">Todas</option>@foreach($matriculas_tasacion as $m)<option value="{{ $m }}" {{ request('matricula') == $m ? 'selected' : '' }}>{{ $m }}</option>@endforeach</select></div>
    ...
    <div class="vx-filtro" data-filtro="valor_min"><label class="vx-filtro-label">Valor mín.</label><input type="number" step="0.01" name="valor_min" class="vx-input" value="{{ request('valor_min') }}"></div>
    <div class="vx-filtro" data-filtro="valor_max"><label class="vx-filtro-label">Valor máx.</label><input type="number" step="0.01" name="valor_max" class="vx-input" value="{{ request('valor_max') }}"></div>
    ...
    <div class="vx-filtro" data-filtro="tasador"><label class="vx-filtro-label">Tasador</label><select name="tasador_id" class="vx-select"><option value="">Todos</option>@foreach($tasadores as $t)<option value="{{ $t->id }}" {{ request('tasador_id') == $t->id ? 'selected' : '' }}>{{ $t->nombre_completo }}</option>@endforeach</select></div>
</x-filtros-avanzados>
