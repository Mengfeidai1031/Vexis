@extends('layouts.app')
@section('title', 'Editar Venta - VEXIS')
@section('content')
<div class="vx-page-header"><h1 class="vx-page-title">Editar: {{ $venta->codigo_venta }}</h1><a href="{{ route('ventas.index') }}" class="vx-btn vx-btn-secondary"><i class="bi bi-arrow-left"></i> Volver</a></div>
<div style="max-width:900px;"><div class="vx-card"><div class="vx-card-body">
    <form action="{{ route('ventas.update', $venta) }}" method="POST">@csrf @method('PUT')
        {{-- Vehículo y Cliente --}}
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0 16px;">
            <div class="vx-form-group"><label class="vx-label">Vehículo <span class="required">*</span></label><select class="vx-select" name="vehiculo_id" id="vehiculoSelect" required>@foreach($vehiculos as $v)<option value="{{ $v->id }}" data-marca="{{ $v->marca_id }}" data-modelo="{{ $v->modelo }}" data-version="{{ $v->version }}" {{ old('vehiculo_id', $venta->vehiculo_id) == $v->id ? 'selected' : '' }}>{{ $v->matricula ?? 'SIN MAT.' }} — {{ $v->marca?->nombre }} {{ $v->modelo }}</option>@endforeach</select><a href="{{ route('vehiculos.create') }}" class="vx-select-create" target="_blank"><i class="bi bi-plus-circle"></i> Crear nuevo</a></div>
            <div class="vx-form-group"><label class="vx-label">Cliente</label><select class="vx-select" name="cliente_id"><option value="">Sin asignar</option>@foreach($clientes as $c)<option value="{{ $c->id }}" {{ old('cliente_id', $venta->cliente_id) == $c->id ? 'selected' : '' }}>{{ $c->nombre }} {{ $c->apellidos }}</option>@endforeach</select></div>
        </div>

        {{-- Marca, Empresa, Centro --}}
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:0 16px;">
            <div class="vx-form-group"><label class="vx-label">Marca</label><select class="vx-select" name="marca_id" id="marcaSelect"><option value="">—</option>@foreach($marcas as $m)<option value="{{ $m->id }}" {{ old('marca_id', $venta->marca_id) == $m->id ? 'selected' : '' }}>{{ $m->nombre }}</option>@endforeach</select></div>
            <div class="vx-form-group"><label class="vx-label">Empresa <span class="required">*</span></label><select class="vx-select" name="empresa_id" id="empresaSelect" required>@foreach($empresas as $e)<option value="{{ $e->id }}" data-cp="{{ $e->codigo_postal }}" {{ old('empresa_id', $venta->empresa_id) == $e->id ? 'selected' : '' }}>{{ $e->nombre }}</option>@endforeach</select></div>
            <div class="vx-form-group"><label class="vx-label">Centro <span class="required">*</span></label><select class="vx-select" name="centro_id" required>@foreach($centros as $c)<option value="{{ $c->id }}" {{ old('centro_id', $venta->centro_id) == $c->id ? 'selected' : '' }}>{{ $c->nombre }}</option>@endforeach</select></div>
        </div>

        {{-- Precios --}}
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0 16px;">
            <div class="vx-form-group"><label class="vx-label">Precio Venta (€) <span class="required">*</span></label><input type="number" class="vx-input" id="precioVenta" name="precio_venta" value="{{ old('precio_venta', $venta->precio_venta) }}" step="0.01" min="0" required style="font-family:var(--vx-font-mono);"></div>
            <div class="vx-form-group"><label class="vx-label">Descuento general (€)</label><input type="number" class="vx-input" id="descuento" name="descuento" value="{{ old('descuento', $venta->descuento) }}" step="0.01" min="0" style="font-family:var(--vx-font-mono);"></div>
        </div>
        ...
        <div class="vx-form-group" style="margin-top:8px;">
            ...
            <div id="extrasContainer">
                @php $idx = 0; @endphp
                @foreach(old('conceptos', $venta->conceptos->toArray()) as $c)
                    @if(($c['tipo'] ?? '') === 'extra')
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:0 16px;">
            <div class="vx-form-group"><label class="vx-label">Forma de Pago</label><select class="vx-select" name="forma_pago" required>@foreach(\App\Models\Venta::$formasPago as $k => $v)<option value="{{ $k }}" {{ old('forma_pago', $venta->forma_pago) == $k ? 'selected' : '' }}>{{ $v }}</option>@endforeach</select></div>
            <div class="vx-form-group"><label class="vx-label">Estado</label><select class="vx-select" name="estado" required>@foreach(\App\Models\Venta::$estados as $k => $v)<option value="{{ $k }}" {{ old('estado', $venta->estado) == $k ? 'selected' : '' }}>{{ $v }}</option>@endforeach</select></div>
            <div class="vx-form-group"><label class="vx-label">Fecha Venta</label><input type="date" class="vx-input" name="fecha_venta" value="{{ old('fecha_venta', $venta->fecha_venta->format('Y-m-d')) }}" required></div>
            <div class="vx-form-group"><label class="vx-label">Fecha Entrega</label><input type="date" class="vx-input" name="fecha_entrega" value="{{ old('fecha_entrega', $venta->fecha_entrega?->format('Y-m-d')) }}"></div>
        </div>
        <div class="vx-form-group"><label class="vx-label">Observaciones</label><textarea class="vx-input" name="observaciones" rows="2">{{ old('observaciones', $venta->observaciones) }}</textarea></div>
        <div style="display:flex;justify-content:flex-end;gap:8px;"><a href="{{ route('ventas.index') }}" class="vx-btn vx-btn-secondary">Cancelar</a><button type="submit" class="vx-btn vx-btn-primary"><i class="bi bi-check-lg"></i> Actualizar</button></div>
    </form>
</div></div></div>
