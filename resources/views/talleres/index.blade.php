    ...
    <div class="vx-table-wrapper"><table class="vx-table">
        ...
        <tr>
            ...
            <td><div class="vx-btn-group">
                <a href="{{ route('talleres.show', $t) }}" class="vx-btn vx-btn-info vx-btn-sm"><i class="bi bi-eye"></i></a>
                @can('editar talleres')<a href="{{ route('talleres.edit', $t) }}" class="vx-btn vx-btn-warning vx-btn-sm"><i class="bi bi-pencil"></i></a>@endcan
                @can('eliminar talleres')<form action="{{ route('talleres.destroy', $t) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Eliminar?');">@csrf @method('DELETE')<button type="submit" class="vx-btn vx-btn-danger vx-btn-sm"><i class="bi bi-trash"></i></button></form>@endcan
            </div></td>
        </tr>@endforeach</tbody>
    </table></div>
