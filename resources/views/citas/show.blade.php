...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;max-width:950px;">
    <div class="vx-card"><div class="vx-card-header"><h2>Información de la Cita</h2></div><div class="vx-card-body">
        <div class="vx-info-row"><div class="vx-info-label">Cliente</div><div class="vx-info-value" style="font-weight:600;">@if($cita->cliente)<a href="{{ route('clientes.show', $cita->cliente) }}">{{ $cita->cliente->nombre }} {{ $cita->cliente->apellidos }}</a> <span style="color:var(--vx-text-muted);font-weight:400;font-size:11px;">(registrado)</span>@else {{ $cita->cliente_nombre ?? '—' }} @endif</div></div>
        <div class="vx-info-row"><div class="vx-info-label">Vehículo</div><div class="vx-info-value">@if($cita->vehiculo)<a href="{{ route('vehiculos.show', $cita->vehiculo) }}">{{ $cita->vehiculo->matricula }} — {{ $cita->vehiculo->modelo }}</a> <span style="color:var(--vx-text-muted);font-size:11px;">(registrado)</span>@else {{ $cita->vehiculo_info ?? '—' }} @endif</div></div>
    ...
    <div class="vx-card"><div class="vx-card-header"><h2>Asignación</h2></div><div class="vx-card-body">
