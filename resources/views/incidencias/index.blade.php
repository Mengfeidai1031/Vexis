...
<div class="vx-page-header">
    <h1 class="vx-page-title">Incidencias</h1>
...
<x-filtros-avanzados :action="route('incidencias.index')">
    <div class="vx-filtro" data-filtro="codigo"><label class="vx-filtro-label">Código</label><select name="codigo_incidencia" class="vx-select"><option value="">Todos</option>@foreach($codigos_incidencia as $c)<option value="{{ $c }}" {{ request('codigo_incidencia') == $c ? 'selected' : '' }}>{{ $c }}</option>@endforeach</select></div>
    <div class="vx-filtro" data-filtro="titulo"><label class="vx-filtro-label">Título</label><select name="titulo" class="vx-select"><option value="">Todos</option>@foreach($titulos_incidencia as $t)<option value="{{ $t }}" {{ request('titulo') == $t ? 'selected' : '' }}>{{ $t }}</option>@endforeach</select></div>
...
<div class="vx-card"><div class="vx-card-body" style="padding:0;">
    ...
    <div class="vx-table-wrapper"><table class="vx-table">
        ...
        <tr>
            ...
            <td><div class="vx-actions"><button class="vx-actions-toggle" aria-label="Abrir acciones" aria-haspopup="menu" aria-expanded="false"><i class="bi bi-three-dots-vertical" aria-hidden="true"></i></button><div class="vx-actions-menu">@can('ver incidencias')
                <a href="{{ route('incidencias.show', $inc) }}"><i class="bi bi-eye" style="color:var(--vx-info);"></i> Ver</a>
            @endcan
            @can('editar incidencias')
                <a href="{{ route('incidencias.edit', $inc) }}"><i class="bi bi-pencil" style="color:var(--vx-warning);"></i> Editar</a>
            @endcan
            @can('eliminar incidencias')
                <form action="{{ route('incidencias.destroy', $inc) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Eliminar esta incidencia?');">@csrf @method('DELETE')
