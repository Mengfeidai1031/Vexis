            ...
            <tr>
                ...
                <td>
                    <div class="vx-btn-group">
                        <a href="{{ route('stocks.show', $s) }}" class="vx-btn vx-btn-info vx-btn-sm"><i class="bi bi-eye"></i></a>
                        @can('editar stocks')<a href="{{ route('stocks.edit', $s) }}" class="vx-btn vx-btn-warning vx-btn-sm"><i class="bi bi-pencil"></i></a>@endcan
                        @can('eliminar stocks')
                        <form action="{{ route('stocks.destroy', $s) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Eliminar?');">@csrf @method('DELETE')<button type="submit" class="vx-btn vx-btn-danger vx-btn-sm"><i class="bi bi-trash"></i></button></form>
                        @endcan
                    </div>
                </td>
            </tr>
