...
class CatalogoPrecioController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        CatalogoPrecio::create($request->only(['marca_id', 'modelo', 'version', 'combustible', 'potencia_cv', 'precio_base', 'precio_oferta', 'disponible', 'imagen_url', 'anio_modelo']));
    ...
    {
        $request->validate(['marca_id' => 'required|exists:marcas,id', 'modelo' => 'required|max:150', 'precio_base' => 'required|numeric|min:0']);
        $catalogo_precio->update([...$request->only(['marca_id', 'modelo', 'version', 'combustible', 'potencia_cv', 'precio_base', 'precio_oferta', 'imagen_url', 'anio_modelo']), 'disponible' => $request->boolean('disponible', true)]);
