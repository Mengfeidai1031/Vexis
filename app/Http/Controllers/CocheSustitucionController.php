...
class CocheSustitucionController extends Controller
{
    public function index(Request $request)
    {
        ...
        $reservas = ReservaSustitucion::with('coche')
            ...
            ->where(function ($q) use ($mes) {
                $q->whereBetween('fecha_inicio', [$mes, $mes->copy()->endOfMonth()])->orWhereBetween('fecha_fin', [$mes, $mes->copy()->endOfMonth()]);
            })
            ->get()->map(fn ($r) => ['title' => $r->coche->matricula.' — '.$r->cliente_nombre, 'start' => $r->fecha_inicio->format('Y-m-d'), 'end' => $r->fecha_fin->addDay()->format('Y-m-d'), 'color' => match ($r->estado) {
                'reservado' => '#f39c12', 'entregado' => '#3498db', default => '#2ecc71'
            }]);
    ...
    public function store(Request $request)
    {
        ...
        CocheSustitucion::create($request->only(['matricula', 'modelo', 'marca_id', 'taller_id', 'empresa_id', 'color', 'anio', 'observaciones']));
    ...
    public function update(Request $request, CocheSustitucion $coches_sustitucion)
    {
        ...
        $coches_sustitucion->update([...$request->only(['matricula', 'modelo', 'marca_id', 'taller_id', 'empresa_id', 'color', 'anio', 'observaciones']), 'disponible' => $request->boolean('disponible', true)]);
